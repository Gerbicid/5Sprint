...
func (t Training) meanSpeed() float64 {
	// вставьте ваш код ниже
	return float64(t.Action) * t.LenStep / MInKm / t.Duration.Hours() //проверить формулу!
}
...
func (t Training) TrainingInfo() InfoMessage {
	...
	return InfoMessage{
		...
		Distance:     t.distance(),
		Speed:        t.meanSpeed(),
		Calories:     t.Calories(), //!возврат под вопросом
	}
}
...
type Running struct {
	// добавьте необходимые поля в структуру
	Training //указал ссылку на структуру
}
...
func (w Walking) Calories() float64 {
	...
	calories := ((CaloriesWeightMultiplier*w.Weight + ((w.meanSpeed()*KmHInMsec)/w.Height/CmInM)*CaloriesSpeedHeightMultiplier*w.Weight) * float64(w.Duration.Hours()) * MinInHours)
	return calories
	//return ((CaloriesWeightMultiplier*Weight + (meanSpeed/60/60**2/Hight)*CaloriesSpeedHeightMultiplier*Weight) * Duration * MinsInHour)
}
